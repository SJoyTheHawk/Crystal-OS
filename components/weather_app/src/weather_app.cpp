...
bool WeatherApp::onCreate()
{
    ...
    lv_obj_t *humidity_caption = makeLabel(humidity_tile, &lv_font_montserrat_14, 0x7C90A4);
    lv_label_set_text(humidity_caption, "HUMIDITY");
    lv_obj_align(humidity_caption, LV_ALIGN_TOP_LEFT, 0, 0);
    humidity_ = makeLabel(humidity_tile, &lv_font_montserrat_28, 0xE6EDF5);
    ...
    lv_obj_t *wind_caption = makeLabel(wind_tile, &lv_font_montserrat_14, 0x7C90A4);
    lv_label_set_text(wind_caption, "WIND");
    lv_obj_align(wind_caption, LV_ALIGN_TOP_LEFT, 0, 0);
    wind_ = makeLabel(wind_tile, &lv_font_montserrat_28, 0xE6EDF5);
    ...
    applyReading(cached);
    if (!cached.valid || nowEpoch() - cached.fetched_at > kStaleSeconds) {
        request();
    }
...
void WeatherApp::applyReading(const Reading &reading)
{
    ...
    const bool expired = reading.fetched_at > 0 &&
                         nowEpoch() - reading.fetched_at > kExpirySeconds;
    lv_obj_set_style_text_color(temperature_, lv_color_hex(expired ? 0x6B7C8D : 0xFFFFFF), 0);
    lv_obj_set_style_text_color(condition_, lv_color_hex(expired ? 0x5E6E7E : 0xC9D6E2), 0);
    lv_obj_set_style_opa(glyph_, expired ? LV_OPA_40 : LV_OPA_COVER, 0);
...
void WeatherApp::setStatus(const Reading &reading, bool expired)
{
    ...
    char text[80];
    if (reading.fetched_at <= 0) {
        text[0] = '\0';
    }
    else {
        const int32_t age = nowEpoch() - reading.fetched_at;
        formatAge(age > 0 ? age : 0, text, sizeof(text));
    }
    ...
    lv_obj_set_style_text_color(status_, lv_color_hex(expired ? 0xC98A3C : 0x7C90A4), 0);
